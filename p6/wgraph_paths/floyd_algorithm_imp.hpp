...
inline std::ostream &print_floyd_I(std::ostream &out, IMatrix const &I) {
  std::string blanks = "          ";
...
inline std::ostream &print_floyd_D(std::ostream &out, FMatrix const &D) {
  std::string blanks = "          ";
  std::string unders = "__________";
  size_t field_width = 4;
  out << blanks.substr(0, field_width + 1);
 ...
 *
 * Also compute two maps to convert between the vertex key and the index of the
 * matrix.
 *
 ...
 */
template <class T>
void compute_weight_matrix(Graph<T, float> const &g, FMatrix &D) {
  D.resize(g.num_vertices(), g.num_vertices(),
           std::numeric_limits<float>::infinity());

  // TODO: Initialize the distances matrix traversing the graph by vertex and
  // edges.
...
template <class T>
void floyd_algorithm(Graph<T, float> const &g, FMatrix &D, IMatrix &I) {
...
std::list<size_t> floyd_path(size_t src, size_t dst, IMatrix const &I) {
